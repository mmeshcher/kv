    ...
    visibleOptions.add_options()
            ("log-level",
             boost::program_options::value<std::string>(
                &Globals::logLevel)->default_value("error"),
             "debug, info, warning or error level")
            ("thread-count",
             boost::program_options::value<size_t>(&Globals::threadAmount)
             ->default_value(std::thread::hardware_concurrency()),
             "Threads amount")
            ("output",
             boost::program_options::value<std::string>(&Globals::output)
...
{
    static const std::string charecters =
